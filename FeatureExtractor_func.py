...
def calc_dist(s1, s2, method, w=None):
    ...
    *TODO: should change dissimilarity to similarity (standard measure)
    ...
    if method == 'dtw_dist' or method == 'dtw_path_len':
        
        n = len(s1)
        m = len(s2)
        assert n == m
        # allocate DTW matrix and fill it with large values
        Dwt = np.ones([n + 1, m + 1]) * float("inf")
        ...
        for i in range(n):
            for j in range(max(0, i - w), min(m, i + w)):
                Dwt[i + 1, j + 1] = cost[i, j] + np.min([Dwt[i, j + 1], 
                                                         Dwt[i + 1, j], 
                                                         Dwt[i, j]])
    
        if method == 'dtw_dist':
            # obtain Euclidean distance
            return np.sqrt(Dwt[-1, -1])
        ...
        if method == 'dtw_path_len':
            ...
            while i > 0 or j > 0:
                if i == 0: 
                    ...
                    last = min(Dwt[i, j], Dwt[i, j+1], Dwt[i+1, j])
                    if Dwt[i, j] == last:
                        i -= 1
                        j -= 1
                    elif Dwt[i+1, j] == last:
