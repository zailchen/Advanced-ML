...
def split(matrix, train_ratio, valid_ratio):
    n = matrix.shape[0]
    index = np.arange(n)
...
if __name__ == "__main__":
    ...
    experiment_cv = 'basc064' # adjust according to needs
    ...
    feature_path = 'fmri_corr_features/1D/' + feature_list[0] + '_corr_1d.pkl' # adjust according to needs
    
    with open(feature_path, 'rb') as f:
        matrix = np.asarray(pickle.load(f))

    X_train, y_train, X_valid, y_valid, X_test, y_test = split(matrix, train_ratio, valid_ratio)
    
    ae1_model_path = format_config("./data/model/{experiment}_autoencoder-1.ckpt", {
        "experiment": experiment_cv,
    })
    ae2_model_path = format_config("./data/model/{experiment}_autoencoder-2.ckpt", {
        "experiment": experiment_cv,
    })
    nn_model_path = format_config("./data/model/{experiment}_mlp.ckpt", {
        "experiment": experiment_cv,
    })
